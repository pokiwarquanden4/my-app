import styles from './Questions.module.scss'
import { useState } from 'react';
import OptionsListButton from '../../Component/OptionsListButton/OptionsListButton';
import TagsComponent from '../../Component/TagsComponent/TagsComponent';

const filterList = [
    {
        name: "Newest"
    },
    {
        name: "Active"
    },
    {
        name: "Unanswered"
    },
    {
        name: "Star"
    },
    {
        name: "Answered"
    },
]

const tagsList = [
    {
        name: "Java"
    },
    {
        name: "Reactjs"
    },
    {
        name: "JavaScripts"
    },
]

function formatTimeAgo(timestamp: Date): string {
    const now = new Date();
    const timeDifference = now.getTime() - timestamp.getTime();
    const seconds = Math.floor(timeDifference / 1000);
    const minutes = Math.floor(seconds / 60);
    const hours = Math.floor(minutes / 60);
    const days = Math.floor(hours / 24);

    if (days > 0) {
        const formattedDate = timestamp.toLocaleString();
        return formattedDate;
    } else if (hours > 0) {
        return `${hours} hour${hours > 1 ? 's' : ''} ago`;
    } else if (minutes > 0) {
        return `${minutes} minute${minutes > 1 ? 's' : ''} ago`;
    } else {
        return `${seconds} second${seconds > 1 ? 's' : ''} ago`;
    }
}

// Sử dụng hàm để tạo chuỗi thời gian
const timestamp = new Date('2023-10-14T12:00:00');


function Questions() {

    return <div className={`${styles.content}`}>
        <div className={`d-flex justify-content-between align-items-center ${styles.content_header}`}>
            <div className={`h4 mb-0 ${styles.content_header_content}`}>All Questions</div>
            <input className="btn btn-primary" type="button" value="Ask Questions"></input>
        </div>
        <div className={`d-flex pb-3 align-items-center justify-content-between pt-5 ${styles.content_filter}`}>
            <div className={`${styles.questions_number}`}>23,938,061 questions</div>
            <div className={`${styles.questions_filter_items}`}>
                <OptionsListButton data={filterList}></OptionsListButton>
            </div>
        </div>
        <div className={`${styles.content_lists}`}>
            <div className={`pb-4 pt-3 d-flex ${styles.content_list}`}>
                <div className={`pe-3 d-flex flex-column justify-content-between ${styles.content_list_info}`}>
                    <div className={`${styles.rate}`}>200 rates</div>
                    <div className={`${styles.answere}`}>300 answeres</div>
                    <div className={`${styles.vertified}`}>20 vertified</div>
                </div>
                <div className={`ps-4 ${styles.content_list_data}`}>
                    <div className={`${styles.content_wrapper}  flex-fill`}>
                        <div className={styles.header}>Why is processing a sorted array faster than processing an unsorted array?</div>
                        <div className={styles.sumary}>Trên bờ biển đầy sóng vỗ, tôi đứng đối diện với mặt biển bao la. Những ngọn sóng cao đánh đập vào bờ cát trắng mịn, tạo ra tiếng sóng biển vang vọng khắp nơi. Bầu trời trong xanh vô tận trải rộng phía trên, và tia nắng ấm áp chiếu xuống, làm cho biển cả trở nên sáng bóng và lấp lánh. Tôi hít thở bầu không khí mặn mà của biển và cảm nhận sự tự do trong từng hơi thở. Cảm giác này khiến tôi thấy minh chứng cho sự kỳ diệu của tự nhiên và sức mạnh của biển cả vô tận.</div>
                        <TagsComponent data={tagsList}></TagsComponent>
                    </div>
                    <div className={`d-flex justify-content-end ${styles.content_footer}`}>
                        <div className={`d-flex align-items-center ${styles.account}`}>
                            <div className={styles.avatar}>
                                <img alt='avatar' className={styles.account_img} src='data:image/png;base64,'></img>
                            </div>
                            <div className={styles.account_name}>pokiwarquanden</div>
                        </div>
                        <div className={`ms-1 d-flex align-items-center ${styles.time}`}>
                            {formatTimeAgo(timestamp)}
                        </div>
                    </div>
                </div>
            </div>
            <div className={`pb-4 pt-3 d-flex ${styles.content_list}`}>
                <div className={`pe-3 d-flex flex-column justify-content-between ${styles.content_list_info}`}>
                    <div className={`${styles.rate}`}>200 rates</div>
                    <div className={`${styles.answere}`}>300 answeres</div>
                    <div className={`${styles.vertified}`}>20 vertified</div>
                </div>
                <div className={`ps-4 ${styles.content_list_data}`}>
                    <div className={`${styles.content_wrapper}  flex-fill`}>
                        <div className={styles.header}>Why is processing a sorted array faster than processing an unsorted array?</div>
                        <div className={styles.sumary}>Trên bờ biển đầy sóng vỗ, tôi đứng đối diện với mặt biển bao la. Những ngọn sóng cao đánh đập vào bờ cát trắng mịn, tạo ra tiếng sóng biển vang vọng khắp nơi. Bầu trời trong xanh vô tận trải rộng phía trên, và tia nắng ấm áp chiếu xuống, làm cho biển cả trở nên sáng bóng và lấp lánh. Tôi hít thở bầu không khí mặn mà của biển và cảm nhận sự tự do trong từng hơi thở. Cảm giác này khiến tôi thấy minh chứng cho sự kỳ diệu của tự nhiên và sức mạnh của biển cả vô tận.</div>
                        <div className={`d-flex pt-2 ${styles.tags}`}>
                            <div className={`${styles.tag}`}>Java</div>
                            <div className={`${styles.tag}`}>JavaScripts</div>
                            <div className={`${styles.tag}`}>Reactjs</div>
                        </div>
                    </div>
                    <div className={`d-flex justify-content-end ${styles.content_footer}`}>
                        <div className={`d-flex align-items-center ${styles.account}`}>
                            <div className={styles.avatar}>
                                <img alt='account' className={styles.account_img} src='data:image/png;base64,'></img>
                            </div>
                            <div className={styles.account_name}>pokiwarquanden</div>
                        </div>
                        <div className={`ms-1 d-flex align-items-center ${styles.time}`}>
                            {formatTimeAgo(timestamp)}
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
}

export default Questions